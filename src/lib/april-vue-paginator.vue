<template>
    <div class='april-paginator-content' v-if="totalRow > 10" :style="{'text-align': textAlign}">
        <div class="paginator">
            <div class="first-page page" @click="setCurrentPage(1)">{{locale == 'zh' ? '首页' : 'First Page'}}</div>

            <div class="icon" @click="setCurrentPage(currentPage-1)" 
            ...
            <div class="page"  v-for="(item,index) in middlePages" :key='index'
            ...
            <div class="last-page page" @click="setCurrentPage(totalPage)">{{locale == 'zh' ? '尾页': 'Last Page'}}</div>

            <div class="current-page"><input type="number" @keydown="onInputKeyDown($event)" v-model="changeCurrentPage"> <span @click="jumpToChangeCurrentPage()">{{locale == 'zh' ? '跳转': 'Go to'}}</span></div>
            <!-- <div class="page-size">****{{totalRow}}****</div> --> 
            <div class="page-size">{{locale == 'zh' ? '每页显示': 'Per Page'}} <input type="number" v-model="changePageSize" @keyup="setPageSize()" >{{locale == 'zh' ? '行': 'Row'}}</div>
        </div>
    </div>
</template>
